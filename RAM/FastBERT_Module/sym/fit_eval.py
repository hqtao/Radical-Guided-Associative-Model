...
def _fit_f(_net, _data, bp_loss_f, loss_function, loss_monitor):
    word, char, associate_word, word_mask, associate_word_mask, label = _data

    output = _net(word, char, associate_word, word_mask, associate_word_mask)
...
def eval_f(_net, test_data, ctx=mx.cpu()):
    ...
    for batch_data in tqdm(test_data, "evaluating"):
        ...
        word, char, associate_word, word_mask, associate_word_mask, label = batch_data
        output = _net(
            word, char, associate_word, word_mask, associate_word_mask,
        )
        pred = mx.nd.argmax(output, axis=1)
        ground_truth.extend(label.asnumpy().tolist())
        prediction.extend(pred.asnumpy().tolist())
