...
def extract(data_src, embeddings):
    ...
    features = [word_feature, char_feature, associate_word_feature]
    ...
    for ds in tqdm(loading(data_src), "loading data from %s" % data_src):
        ...
        c = token_to_idx(embeddings["c"], ds["c"])
...
def transform(raw_data, params):
    ...
    word_feature, char_feature, associate_word_feature = features
...
if __name__ == '__main__':
    ...
    total_len = 0
    total_count = 0
    for data in tqdm(loading(filename)):
        total_count += 1
        total_len += len(caw("".join(data["c"])))
